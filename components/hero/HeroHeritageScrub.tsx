...
// 헤리티지 이미지 9장(Frame 12~20) + CLI 리빌 1 beat + 홀드 버퍼 2 beats = 12 beats
const SCRUB_BEATS = 9; // 헤리티지 이미지 스크럽 구간
const REVEAL_BEATS = 1; // CLI crossfade 구간
const HOLD_BEATS = 2; // CLI 홀드 버퍼
const HERO_BEATS = SCRUB_BEATS + REVEAL_BEATS + HOLD_BEATS; // 12

const TOTAL = SCRUB_BEATS; // 슬라이더 슬롯 수 = 9 (CLI는 슬라이더에 포함되지 않음)
const IMAGES = Array.from(
  { length: TOTAL },
  (_, i) => `/heritage/${String(i + 1).padStart(2, "0")}.png`,
);

// 스크럽 단계가 전체 progress에서 차지하는 비율
// progress ∈ [0, SCRUB_RATIO] → 이미지 스크럽
// progress ∈ [SCRUB_RATIO, 1] → CLI 크로스페이드
const SCRUB_RATIO = SCRUB_BEATS / (SCRUB_BEATS + REVEAL_BEATS); // 9/10 = 0.9
...
// 스크럽 단계의 floatIndex 기반 카피 인덱스 (0..2만 반환)
const copyIndexForScrub = (floatIndex: number): number => {
...
export function HeroHeritageScrub() {
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      // 스크럽 + 리빌 = 10 beats, 홀드 = 2 beats → 앞 10/12만이 useful
      const usefulEnd = (SCRUB_BEATS + REVEAL_BEATS) / HERO_BEATS;
  ...
  // 스크럽 페이즈 (0..1) — progress의 앞부분을 이미지 스크럽에 할당
  const scrubPhase = Math.min(1, progress / SCRUB_RATIO);
  // 리빌 페이즈 (0..1) — progress의 뒷부분을 CLI 크로스페이드에 할당
  const revealPhase = Math.max(
    0,
    Math.min(1, (progress - SCRUB_RATIO) / (1 - SCRUB_RATIO)),
  );

  // 자석 스냅 (스크럽 페이즈에만 적용)
  const rawFloat = scrubPhase * (TOTAL - 1);
  ...
  // 카피: 리빌 페이즈가 시작되면(> 0) 즉시 "RelayAX" 카피로 전환
  const copyIdx = revealPhase > 0 ? 3 : copyIndexForScrub(floatIndex);
  ...
  // CLI 크로스페이드
  const cliOpacity = revealPhase;
  const sliderOpacity = 1 - revealPhase;
        ...
        <div className="relative mx-auto grid w-full max-w-7xl flex-1 grid-cols-1 items-center gap-10 px-6 md:grid-cols-2 md:gap-16 md:px-10">
          ...
          */}
          <div className="relative h-[62vh] w-full">
            {/* 슬라이더 — 헤리티지 이미지 9장만 포함. 리빌 페이즈에 fade-out */}
            <div
            ...
            {/* CLI 리빌 — 헤리티지 스크럽과 분리된 독립 레이어.
                리빌 페이즈에서 opacity 0→1 + scale 0.96→1 + translateY 8→0 로 부드럽게 등장 */}
            <div
              className="pointer-events-none absolute inset-0 flex items-center justify-center"
              style={{
                opacity: cliOpacity,
                transform: `translate3d(0, ${(1 - cliOpacity) * 8}px, 0) scale(${0.96 + cliOpacity * 0.04})`,
            ...
            >
              <div
                ...
                style={{
                  filter:
                    "drop-shadow(0 30px 40px rgba(30,20,10,0.22)) drop-shadow(0 8px 14px rgba(30,20,10,0.10))",
                }}
              >
                <CliTerminal />
              </div>
